...
export function CareersPage() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/careers`); // PUBLIC ROUTE: Bina token ke
  ...
  return (
    <>
      ...
      <section className="py-16 md:py-20 bg-slate-50">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {why.map((w, i) => (
              <motion.div key={w.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group rounded-2xl border border-border bg-white p-6 shadow-soft transition-all hover:-translate-y-1 hover:border-primary hover:shadow-md cursor-pointer">
            ...
            <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {benefits.map((b) => (
                <li key={b} className="flex items-center gap-2 text-xs font-medium text-navy/80 group">
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-white"><Heart className="h-3 w-3" /></span>
                  {b}
                </li>
              ))}
            </ul>
      ...
      {/* DYNAMIC OPEN POSITIONS - YAHAN FIX KIYA HAI */}
      <section id="openings" className="py-16 md:py-24 bg-slate-50">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="space-y-4">
            {jobsList.map((j, i) => (
              <motion.div key={j.slug || j.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-xl border border-border bg-white p-5 shadow-sm transition hover:border-primary hover:shadow-md">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] font-medium text-muted-foreground">
                    ...
                    {/* FIX: employmentType */}
                    <span className="inline-flex items-center gap-1 bg-secondary px-2 py-1 rounded"><Clock className="h-3 w-3" /> {j.employmentType}</span>
                    <span className="bg-primary/10 text-primary px-2 py-1 rounded">{j.experience}</span>
                  </div>
                  {/* FIX: shortDescription */}
                  <p className="mt-3 max-w-2xl text-xs text-muted-foreground line-clamp-2">{j.shortDescription}</p>
                </div>
